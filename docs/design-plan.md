...
| Find (rev 6) | 4 | the column moves on its own, not with the scroll: once the section is in view and the reader scrolls, a separate rAF loop races a 54-sheet set (tiled twice) upward at a constant 14 rows a second with the vertical blur at its full 4px, so a slow scroll never reads as lag. When step 2 reaches the centre band it decelerates onto the resting frame in about 600ms (a cubic that starts at the racing speed and lands exactly on the frame, the blur falling with the speed); only then do the three matches lift and the other 33 drop to 25%. Steps 3 and 4 stay scroll-driven. The loop pauses off screen; phones and reduced motion keep the still 6 × 6 wall |
| Verify (rev 6) | timed, 5 × 4s, looping, one screen | no scroll: the whole section fits a 900px-tall viewport. Five rule headings stacked; a thin orange line under the active one fills over 4s and is gone the moment the rule completes, so only the active rule ever has a line. The next lights, and after the fifth it rolls back to the first, for as long as the section is in view. The pane crossfades a small figure of what the rule checks with its description and verdict (350ms). Pauses off screen and resumes where it was; a click on a heading jumps there and restarts its line. The coverage bar is the summary under the list, always resolved, not a step. Reduced motion: no timer, a click or focus picks the rule |
