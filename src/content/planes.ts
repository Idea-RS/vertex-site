...
/** What each plane of a sheet is, and what Vertex does with it. One step per plane in the explainer. */
export const planeDescriptions: Record<PatentPlane, { title: string; what: string; vertex: string }> = {
  "border-zones": {
    ...
    vertex: "Every finding Vertex reports is placed by zone, so a reviewer opens the sheet and goes straight to C4.",
  },
  geometry: {
    title: "Geometry",
  ...
  "tables-notes": {
    ...
    what: "A variant table defines the sheet's variables; the notes carry the house rules that never make it into a dimension. Together they are the sheet's small print.",
