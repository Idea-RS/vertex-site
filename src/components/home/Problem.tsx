    ...
    <section className="rule section">
      <div className="container">
        <blockquote>
          ...
          <footer className="mt-5 text-small text-vx-400">
            An engineering manager at our design partner. Their archive: 6,926 drawings, three decades of revisions.
          </footer>
        </blockquote>
        ...
        <div className="mt-16 grid gap-10 sm:grid-cols-3 lg:mt-24 lg:gap-8">
          {problemStats.map((s, i) => (
            <div key={s.label} className="relative flex items-stretch gap-6">
              <div ref={i === 2 ? stat : undefined} className="min-w-0 flex-1 border-l border-vx-600 pl-6">
                <div className="mono text-[clamp(2.5rem,4.2vw,3.5rem)] leading-none text-vx-100">{s.value}</div>
                <p className="mt-3 max-w-[22ch] text-small text-vx-400">{s.label}</p>
              </div>
              {i === 2 && <Dim axis="y" measure={stat} className="shrink-0" />}
            </div>
          ))}
        </div>
        <p className="mt-10 max-w-[60ch] text-small text-vx-400">
